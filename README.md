...
- **球员历史快照**（V2.0 新增）— 每个球员可保留多条历史快照，追踪长期成长
  - 弹窗顶部 "📜 历史" 按钮 → 历史快照面板
  - 输入球员 ID 或点击主列表球员项 → 查询所有快照
  - 显示快照编号、采集时间、技能简写、年龄/薪金/位置/潜力
- **数据导入** — 支持 JSON / SQLite 文件导入，用于重装系统或更换浏览器后恢复数据
- **页面内本地数据浮窗（双列横向柱形图）** — 在球员详情和球队球员列表页面，对页面上看不到技能数据的球员（属性未公开）注入本地面板，以双列横向柱形图展示 10 项技能值，4 档梯度配色（弱/中/强/极强）一眼可比
- **一键导出** — JSON / SQLite 两种格式
- **100% 隐私** — 所有数据存储在浏览器本地 IndexedDB，不上传任何服务器

### ⚠️ V2.0 破坏性变更（v2 用户必读）

升级到 V2.0 会清空原有 IndexedDB 数据，原因是 IndexedDB schema 升级限制：
- v2 `players` store 主键是 `id`（每球员一条）；v3 改为 `recordId` 自增（每球员多条）
- IndexedDB 不允许两个同名 store 同时存在，schema 升级只能破坏性重建

**升级前必须导出 JSON 备份**：
1. 升级前打开插件 → 点击"JSON"导出
2. 安装新版本（自动重建 v3 schema 空数据库）
3. 通过"导入 JSON"恢复数据（自动适配 v3 多快照模式）
...
4. 点击顶部"📜 历史"按钮查看球员历史快照
5. 点击"导出 JSON"或"导出 SQLite"保存数据；点击"导入 JSON"或"导入 SQLite"恢复数据
